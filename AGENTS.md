...
`src/index.ts` is the CLI entrypoint. Core sync logic lives in `src/services/`: `serial.service.ts` handles series folders, `video.service.ts` handles flat video sync, `report.service.ts` writes reports, and `logger.service.ts` centralizes Winston logging. Settings loaders and types are under `src/services/settings/`, while static runtime inputs live in `src/config/` (`setting.json`, `soap.csv`). Build output goes to `dist/`; runtime logs go to `logs/`; sample or working data belongs in `data/`.
...
## Testing Guidelines
There is no automated test suite in the repository yet. Until one is added, validate changes with `npx eslint .`, `npm run build`, and a manual run against safe local folders configured in `src/config/setting.json`. When adding tests, place them near the related module or under a dedicated `tests/` folder and use `*.test.ts` names.
...
Do not commit real source or destination paths in `src/config/setting.json` or secrets in `.env`. Test destructive flows, especially video sync, with disposable directories because copied video files are removed from the source after success.
